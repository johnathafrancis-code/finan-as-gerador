...
import { Home, ReceiptText, Plus, HeartHandshake, Settings2 } from 'lucide-react';

interface MobileBottomNavProps {
  currentTab: 'dashboard' | 'transactions' | 'categories' | 'settlement' | 'settings';
  setCurrentTab: (tab: 'dashboard' | 'transactions' | 'categories' | 'settlement' | 'settings') => void;
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-lg border-t border-slate-200/90 shadow-lg">
      <div className="max-w-md mx-auto px-3 py-1 flex items-center justify-around h-16">
        ...
        >
          <Settings2 className={`w-5 h-5 ${currentTab === 'settings' ? 'stroke-[2.5]' : 'stroke-[1.75]'}`} />
